...
if (isset($_POST["submit_image"])) {
	...
	} elseif (($_FILES["img_file"]["size"] <= 0 ) && ($_FILES["img_file"]["size"] <= 0 )) {
		$msg = errorMessage( "Image is not proper.");
	} else {
		$ext = strtolower(end(explode(".", $img_file)));
		$ext1 = strtolower(end(explode(".", $img_file2)));


		if ( (!in_array($ext, $validExt)) && (!in_array($ext, $validExt) ))  {
			$msg = errorMessage( "Not a valid image file");
		} else {
			$filePath = $folderName. rand(10000, 990000). '_'. time().'.'.$ext;
			$filePath1 = $folderName. rand(10000, 990000). '_'. time().'.'.$ext1;
			
			if ( (move_uploaded_file( $_FILES["img_file"]["tmp_name"], $filePath)) && (move_uploaded_file( $_FILES["img_file2"]["tmp_name"], $filePath1))) {
				$sql = "INSERT INTO image_demo3 VALUES (NULL, '".prepare_input($filePath) ."','".prepare_input($filePath1) ."')";
    ...
    <article> <?php echo $msg; ?>
      ...
      <form method="post" action="" name="f" enctype="multipart/form-data" onSubmit="return validateImage();" >
        ...
        <tr>
            ...
            <td align="center"><img src="<?php echo stripslashes($rs->image_url); ?>" alt="" width="200" height="200"> </td>   
            <td align="center"><img src="<?php echo stripslashes($rs->image_url2); ?>" alt="" width="200" height="200"> </td>  
        </tr>
        <?php 
		}
		?>
